...
(fn () =>
    ...
    in
        ...
        test "4.1"
             (fn () => null (S.connected_components Graph.empty));

        test "4.2"
             (fn () => length (S.connected_components g1) = 3);

        test "4.3"
             (fn () => length (S.connected_components g2) = 1);

        test "4.4"
             (fn () => length (S.connected_components g3) = 1)
